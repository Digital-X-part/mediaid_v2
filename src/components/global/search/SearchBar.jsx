...
const SearchBar = () => {
  ...
  return (
    <>
      {data.openSearchList ?
        <div onClick={() => setData(s => ({ ...s, openSearchList: false }))} className="absolute top-0 left-0 w-full h-screen  "></div>
        : null
      }
      <div className="mt-4 rounded-sm outline-transparent relative mb-1 lg:mb-0 ">
        <div className="flex items-center justify-between mx-1 my-2 rounded-md bg-gray-100 ">
          ...
          <input
            tabIndex={0}
            // onMouseLeave={() => setData(s => ({ ...s, openSearchList: false }))}
            onClick={() => setData(s => ({ ...s, openSearchList: true }))}
        ...
        {data.openSearchList ? (
          <>
            <div className="hidden absolute top-0 mt-12 bg-white shadow-teal-500 rounded-sm w-full md:flex flex-col z-50">
